...
class DBVersion1 {
  static Future<void> dbUpdatesVersion(Database db) async {
    ...
    await db.execute(
        "CREATE TABLE card_transactions ( id INTEGER PRIMARY KEY AUTOINCREMENT, description TEXT NOT NULL, date INTEGER NOT NULL, amount REAL NOT NULL, type INTEGER NOT NULL, category_id INTEGER NOT NULL, card_id INTEGER NOT NULL, created_at INTEGER NOT NULL, FOREIGN KEY (category_id) REFERENCES categories (id), FOREIGN KEY (card_id) REFERENCES cards (id) )");
    await db.execute(
